...
describe("tournamentRules", () => {
  ...
  it("blocks additional registrations once room capacity is reached", () => {
    expect(() => ensureTournamentHasCapacity("Draft", MAX_TOURNAMENT_PLAYERS)).toThrow(/capacity is/);
    expect(() => ensureTournamentHasCapacity("Modern", MAX_TOURNAMENT_PLAYERS)).toThrow(/capacity is/);
  });
