...
export function StandingsTable({
  ...
  useEffect(() => {
    if (selectedRound === null) {
      ...
      const prevRound = finishedRounds.length >= 2
        ? finishedRounds[finishedRounds.length - 2]
        : null;
      if (prevRound !== null && prevRound !== prevRoundRef.current) {
        prevRoundRef.current = prevRound;
        api.getStandings(tournamentId, prevRound).then(setPrevStandings).catch(() => setPrevStandings(null));
    ...
    const fetchPrev = prevRound !== null ? api.getStandings(tournamentId, prevRound) : Promise.resolve(null);
    ...
    Promise.all([fetchCurrent, fetchPrev])
      ...
      .catch(() => { setHistoricalStandings(null); setPrevStandings(null); })
      .finally(() => setLoading(false));
  }, [selectedRound, tournamentId, finishedRounds]);

  const displayedStandings = selectedRound === null ? standings : (historicalStandings ?? standings);
            ...
            <tbody>
              {displayedStandings.map((standing) => (
                ...
                >
                  ...
                  {prevStandings && (
                    <td className="px-3 py-3 text-center">{rankDiff(standing)}</td>
                  )}
                  <td className="px-4 py-3 font-medium text-gray-800">
                    {standing.player.playerId ? (
                      <Link to={`/players/${standing.player.playerId}`} className="hover:text-blue-600 hover:underline">
                  ...
                  <td className="px-4 py-3 text-center font-bold text-gray-800">{standing.matchPoints}</td>
                  <td className="px-4 py-3 text-center text-gray-600">
                    {standing.matchWins}-{standing.matchLosses}-{standing.matchDraws}
                  </td>
                  <td className="px-4 py-3 text-center text-gray-500">{(standing.omwPercent * 100).toFixed(1)}%</td>
                  <td className="px-4 py-3 text-center text-gray-500">{(standing.gwPercent * 100).toFixed(1)}%</td>
                  <td className="px-4 py-3 text-center text-gray-500">{(standing.ogwPercent * 100).toFixed(1)}%</td>
                </tr>
              ))}
            </tbody>
          </table>
