...
export function PairingsTable({ matches, canReport, bestOfFormat = "BO3", onUpdate }: PairingsTableProps) {
  ...
  const resultLabel = (match: MatchDetail) => {
    ...
    if (match.result === "DRAW") {
      return <span className="text-gray-500 text-xs font-semibold">Draw {match.wins1}–{match.wins2}</span>;
    }
  ...
  const renderForm = (match: MatchDetail) => {
    ...
    if (selectedWinner === null) {
      ...
      return (
        <div className="flex items-center gap-1 justify-center flex-wrap">
          ...
          <button onClick={closeForm} className="text-gray-400 hover:text-gray-600 px-2 py-1 text-xs">Cancel</button>
        </div>
      );
    }
    ...
    if (selectedWinner === "draw") {
      return (
        <div className="flex items-center gap-2 justify-center flex-wrap">
          ...
          <Stepper value={winsW} min={0} max={99} onChange={(value) => { setWinsW(value); setWinsL(value); }} />
          <button
          ...
          <button onClick={() => setSelectedWinner(null)} className="text-gray-400 hover:text-gray-600 px-2 py-1 text-xs">← back</button>
        </div>
      );
    }
    ...
    return (
      <div className="flex items-center gap-1.5 justify-center flex-wrap">
        <span className="text-xs font-semibold text-green-700 max-w-[72px] truncate">⭐ {winnerName}</span>
          ...
          {matches.map((match) => (
            <tr key={match.id} className="border-t border-gray-100 hover:bg-gray-50">
              ...
              <td className={`px-4 py-3 text-right font-medium ${match.result === "P2_WIN" ? "text-green-700" : "text-gray-500"}`}>
                {match.player2 ? renderPlayerName(match.player2.name, match.player2.playerId) : <span className="text-gray-300">BYE</span>}
